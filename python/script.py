...
import pandas as pd
import os
train_df = pd.read_csv('python/input/train.csv', index_col="Id", nrows=100)
test_df = pd.read_csv('python/input/test.csv', index_col="Id", nrows=100)
...
def findNext(seq, trie):
    while True:
        ...
        item = next(list, None)
        if item != None:
            best = reduce(lambda a, b: a if a[1] > b[1] else b if b[1] > a[1] else a if len(
                b[0]) <= len(a[0]) else b, list, item)
